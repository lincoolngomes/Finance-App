...
async function diagnosticoCategoriasResgateInvest() {
  // Busca todas as categorias que contenham "Resgate" ou "Invest" no nome
  const { data: categorias, error: catError } = await supabase
    .from('categorias')
    .select('id, nome, tipo, userid')
    .or('nome.ilike.%Resgate%,nome.ilike.%Invest%');
  if (catError) {
    console.error('Erro ao buscar categorias:', catError);
    return;
  }
  if (!categorias || categorias.length === 0) {
    console.log('Nenhuma categoria com "Resgate" ou "Invest" encontrada.');
    return;
  }
...
diagnosticoCategoriasResgateInvest().then(() => process.exit(0));

export default diagnosticoCategoriasResgateInvest;
