...
async function diagnosticoCategoriasInvestimentos() {
  // Busca todas as categorias que contenham "Investimento" no nome
  const { data: categorias, error: catError } = await supabase
    .from('categorias')
    .select('id, nome, tipo, userid')
    .ilike('nome', '%Investimento%');
  if (catError) {
    console.error('Erro ao buscar categorias:', catError);
    return;
  }
  if (!categorias || categorias.length === 0) {
    console.log('Nenhuma categoria com "Investimento" encontrada.');
    return;
  }
...
diagnosticoCategoriasInvestimentos().then(() => process.exit(0));

export default diagnosticoCategoriasInvestimentos;
